...
export function SwotMobileCard({ title, gradient, icon, items, onClick }: SwotMobileCardProps) {
  return (
    <button
      onClick={onClick}
      className={`w-full text-left p-6 rounded-2xl bg-gradient-to-br ${gradient} border border-white/10 shadow-lg relative overflow-hidden group`}
    >
      <div className="absolute top-0 right-0 w-32 h-32 bg-white/10 rounded-full -translate-y-16 translate-x-16 blur-2xl group-hover:scale-150 transition-transform duration-500" />
